...
and table_name = 'c_spire_wireless_lte'
...
#    iGrid = """select max(id) + 1 as id from swat.test_grid"""
#    cur.execute(iGrid)
#    iGrid = cur.fetchone()
#    iGrid = iGrid[0]
#    if iGrid is None:
#        iGrid = 0
    ...
    for vRec in sProviderVertices:
        ...
        fishnet = """insert into swat.test_grid
                (select st_multi(st_setsrid(st_createfishnet(4,4,%s,%s,%s,%s),4326)) as geom);""" % (xRange,yRange,extent[0],extent[1])
    ...
    createTable = """CREATE TABLE swat._agg_%s(
                      gid serial NOT NULL,
                      ...
                      CONSTRAINT _agg_%s_pkey PRIMARY KEY (gid))
